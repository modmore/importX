    ...
    function process() {
        ...
        foreach ($headings as $h) {
            ...
            if (!in_array($h,$fields)) {
                if (substr($h,0,2) != 'tv') {
                    $this->importx->errors[] = $this->modx->lexicon('importx.err.invalidfield', ['field' => $h]);
                }
                else {
                	$tvInHeadings = true;
                    if (intval(substr($h,2)) <= 0) {
                        $this->importx->errors[] = $this->modx->lexicon('importx.err.intexpected', ['field' => $h, 'int' => substr($h,2)]);
                    }
                    else {
                        $tvo = $this->modx->getObject('modTemplateVar',substr($h,2));
                        if (!$tvo) {
                            $this->importx->errors[] = $this->modx->lexicon('importx.err.tvdoesnotexist', ['field' => $h, 'id' => substr($h,2)]);
                        }
                    }
                }
            }
        ...
        foreach ($lines as $line => $lineval) {
            ...
            if ($headingcount != count($curline)) {
                ...
                if (!isset($lines[$line]['context_key'])) {
                    $lines[$line]['context_key'] = $this->importx->defaults['context_key'];
                }

                if (!isset($lines[$line]['parent'])) {
                    $lines[$line]['parent'] = $this->importx->defaults['parent'];
                }

                if (!isset($lines[$line]['published'])) {
                    $lines[$line]['published'] = $this->importx->defaults['published'];
                }
                ...
                if (!isset($lines[$line]['hidemenu'])) {
                    $lines[$line]['hidemenu'] = $this->importx->defaults['hidemenu'];
                }
        ...
        if (count($err) > 0) {
            $this->importx->errors[] = $this->modx->lexicon('importx.err.elementmismatch', ['line' => implode(', ', $err)]);
            return false;
        }
        return $lines;
    }
